...
outName = re.sub('final_dataset_', '', os.path.basename(fname).split('.')[0]) + '_simple_onlyUrban'
...
if purpose == 'cv':
    ...
    #         ALE
    #################################################################################################
    from src.ale import aleplot_1D_continuous
    ...
    for climate in df.climate_label.unique():
        ...
        res_df = aleplot_1D_continuous(train_set_rep, predictors, ml, feature, grid_size = 100, log = True)
        res_df['climate'] = climate
        res_df_all.append(res_df)
    res_df_all = pd.concat(res_df_all).reset_index()
    res_df_all.to_csv(f'../results/ale_{feature}_{model}_{outName}.csv', index = False)
