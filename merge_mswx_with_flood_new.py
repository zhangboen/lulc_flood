...
def read(year):
    ...
    df = []
    for name in ['P','Tmax','Tmin','SWd']:
        df0 = pd.read_csv(f'../data_mswx/MIX_catch_ave_std_{name}_{year}.csv')
        df0 = df0.loc[df0.stat=='ave',:].drop(columns=['stat']).melt(id_vars = 'time')
        df0 = df0.rename(columns={'variable':'ohdb_id','value':name.lower()}).set_index(['time','ohdb_id'])
        df.append(df0)
...
df_meteo.groupby('ohdb_id').p_apply(lambda x:x.drop(columns=['ohdb_id']).rename(columns={'time':'date'}).to_csv(f'../data_mswx/mswx_each_basin/{x.ohdb_id.values[0]}_MIX_meteo_1981-2023.csv', index = False))
...
# print(df2.shape)
# df2.to_csv(f'../data/Qmax_{tmp}_multi_MSWX_MSWEP_GLEAM.csv', index = False)